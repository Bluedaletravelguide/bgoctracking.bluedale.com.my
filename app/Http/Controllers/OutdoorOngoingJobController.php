...
class OutdoorOngoingJobController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        return view('dashboard.outdoor', [
            ...
            'canEdit'     => Auth::user()->can('outdoor.edit'),
        ]);
    }
    ...
    {
        // Authorization check - ensure user has outdoor.edit permission
        if (!Auth::user()->can('outdoor.edit')) {
            Log::warning('=== OUTDOOR UPSERT UNAUTHORIZED ===', [
                'user_id' => Auth::id(),
                'email' => Auth::user()->email ?? 'unknown'
            ]);
            return response()->json([
                'ok' => false,
                'message' => 'You do not have permission to edit outdoor items.'
            ], 403);
        }

        Log::info('=== OUTDOOR UPSERT DEBUG START ===', ['raw' => $req->all()]);

        logger()->info('=== OUTDOOR UPSERT LOG START ===', ['raw' => $req->all()]);
