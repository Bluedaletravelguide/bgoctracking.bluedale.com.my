...
class OutdoorWhiteboardController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        // Determine if current user can edit (CRUD operations)
        $canEdit = false;
        if (Auth::check()) {
            $user = Auth::user();
            if (method_exists($user, 'roles')) {
                $userRoles = $user->roles()->pluck('name')->toArray();
                $canEdit = count(array_intersect($userRoles, ['admin', 'superadmin', 'support'])) > 0;
            } elseif (method_exists($user, 'hasRole')) {
                $canEdit = $user->hasRole(['admin', 'superadmin', 'support']);
            }
        }
        ...
        return view('outdoor.whiteboard', compact(
            ...
            'canEdit' // pass edit permission flag
        ));
    }
