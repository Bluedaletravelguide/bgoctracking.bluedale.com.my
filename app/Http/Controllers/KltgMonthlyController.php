...
class KltgMonthlyController extends Controller
{
    ...
    // {
    //     logger()->info('KLTG Monthly index view requested');
    ...
    public function index(Request $request)
    {
        ...
        if ($masterIdsForYear->isEmpty()) {
            ...
            $baseRowsQuery = MasterFile::query()
                ->select([
                    'id',
                    'company',
                    'product',
            ...
            // Apply same filters to the base rows query
            if ($filterMonth) {
                $baseRowsQuery->where('month', $filterMonth);
            }

            if ($filterCompany) {
                $baseRowsQuery->where('company', $filterCompany);
            }

            if ($filterSearch) {
                $baseRowsQuery->where(function ($q) use ($filterSearch) {
                    $q->where('company', 'LIKE', "%{$filterSearch}%")
                        ->orWhere('product', 'LIKE', "%{$filterSearch}%");
                });
            }
            ...
            if ($filterDateFrom) {
                $baseRowsQuery->whereDate('date', '>=', $filterDateFrom);
            }

            if ($filterDateTo) {
                $baseRowsQuery->whereDate('date_finish', '<=', $filterDateTo);
            }
    ...
    public function exportMatrix(Request $req)
    {
        ...
        $parseMonth = function ($v) {
            ...
            try {
                return (int)Carbon::parse("1 " . $v . " 2000")->format('n');
            } catch (\Throwable $e) {
                return null;
            }
        };
