    ...
    <div class="paper min-h-screen py-6">
        <div class="mx-auto max-w-7xl px-4">
            <div class="mb-5 flex items-center justify-between">
                <div>
                    <h1 class="text-2xl font-semibold ink">Monthly Ongoing Job Calendar</h1>
                    <p class="text-sm text-neutral-500">All Monthly Ongoing Job dates across Outdoor • Media • KLTG</p>
                </div>
                <div class="text-sm">
                ...
                <form id="filters" class="grid grid-cols-1 md:grid-cols-5 gap-3">
                    ...
                    <div>
                        ...
                        <input type="number" name="year" min="2000" max="2100"
                            class="w-full border hairline rounded-md p-2" placeholder="e.g. 2025">
                    </div>
                    <div>
                        <label class="small-caps text-neutral-600">Month</label>
                        <input type="number" name="month" min="1" max="12"
                            class="w-full border hairline rounded-md p-2" placeholder="1-12">
                    </div>
                    <div class="md:col-span-2">
                        <label class="small-caps text-neutral-600">Company / Client</label>
                        <input type="text" name="company" class="w-full border hairline rounded-md p-2"
    ...
    <div id="eventModal" class="fixed inset-0 z-50 hidden">
        ...
        <div class="relative mx-auto mt-16 w-[92%] max-w-xl rounded-2xl bg-white shadow-2xl ring-1 ring-black/5">
            <div class="flex items-start justify-between p-5 border-b border-neutral-200/70">
                <div>
                    <h3 class="text-lg font-semibold ink">Monthly Ongoing Job Details</h3>
                ...
                <button
                    class="rounded-full border border-neutral-300 w-9 h-9 grid place-content-center text-neutral-600 hover:bg-neutral-100"
            ...
            <div class="p-5 space-y-4">
                <div>
                    ...
                    <div id="modal-company" class="text-lg font-medium">—</div>
                </div>
                <div>
                    <div class="small-caps text-neutral-600">Product</div>
                    <div id="modal-title" class="text-lg font-medium">—</div>
                </div>
                <div id="row-site" class="hidden">
                    <div class="small-caps text-neutral-600">Site</div>
                    <div id="modal-site" class="text-lg font-medium">—</div>
                </div>
                <div id="row-month">
                    <div class="small-caps text-neutral-600">Month</div>
                    <div id="modal-month" class="text-lg font-medium">—</div>
                </div>
                <div id="row-category">
                    <div class="small-caps text-neutral-600">Category</div>
                    <div id="modal-category" class="text-lg font-medium">—</div>
                </div>
                <div>
                    <div class="small-caps text-neutral-600">Status</div>
                    <span id="modal-milestone"
                        class="inline-flex px-2 py-1 rounded-full text-lg font-medium bg-amber-100 text-amber-800">—</span>
                </div>
                <div>
                    <div class="small-caps text-neutral-600">Date</div>
                    <div id="modal-date" class="text-lg font-medium">—</div>
                </div>
                <div id="row-subcategory" class="hidden">
                    <div class="small-caps text-neutral-600">Category</div>
                    <div id="modal-subcategory" class="text-lg font-medium">—</div>
                </div>
            </div>

            <div class="flex items-center justify-end gap-2 p-5 border-t border-neutral-200/70">
                <a id="modal-link" href="#" target="_blank" class="hidden primary-btn">Open Detail</a>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            ...
            const calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                events: {
                    url: "{{ route('calendar.coordinators.monthlyongoingjobs') }}",
                    failure: function() {
                ...
                eventClick: function(info) {
                    ...
                    // Milestone display: prefer `title_raw` + month, then append formatted date
                    const monthLabel = (function() {
                        if (!p.month) return '';
                        const m = (typeof p.month === 'number') ? p.month : parseInt(p.month,
                            10);
                        if (!isNaN(m) && m >= 1 && m <= 12) {
                            return new Intl.DateTimeFormat('en', {
                                month: 'short'
                            }).format(new Date(2020, m - 1, 1)).toUpperCase();
                        }
                        return String(p.month).toUpperCase();
                    })();

                    const dateStr = formatDate(info.event.start);
                    const titlePart = p.category ? `${p.category}` : '';
                    const monthPart = monthLabel ? ` ${monthLabel}` : '';
                    const milestoneBuilt = p.combined_value ?
                        `${p.combined_value}` : ('—');
                    setText('#modal-milestone', milestoneBuilt);
                    ...
                    // Category: prefer `subcategory`, fall back to monthly `field_key`
                    const subcategory = (p.subcategory || p.field_key || '').toString().trim();
                    toggleRow('#row-subcategory', !!subcategory);
                    setText('#modal-subcategory', subcategory);

                    // Month and Field Type (monthly details)
                    toggleRow('#row-month', !!p.month);
                    setText('#modal-month', p.month || '—');

                    toggleRow('#row-category', !!p.category);
                    setText('#modal-category', p.category || '—');

                    // Value: show text value for text fields, otherwise show formatted date
                    const valueDisplay = (p.field_type === 'text') ? (p.value_text || '—') : formatDate(
                        info.event.start);
                    toggleRow('#row-value', !!valueDisplay && valueDisplay !== '—');
                    setText('#modal-value', valueDisplay);
